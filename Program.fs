...
let displayJustified x y color (mensaje:string) =
    ...
    Utils.displayMessage nuevaX y color mensaje

let displayAlien state =
    match state.AlienState with
    | Vivo -> Utils.displayMessage state.AlienX state.AlienY ConsoleColor.Yellow "👽"
    | Colisionado -> Utils.displayMessage state.AlienX state.AlienY ConsoleColor.Yellow "⚰️"
    | _ -> ()
    state
...
let displayMisil state =
    state.Misiles
    |> List.iter ( fun m -> Utils.displayMessage m.X m.Y ConsoleColor.Cyan "=>")
    state

let displayEnemyMisil state =
    state.EnemyMisiles
    |> List.iter ( fun m -> Utils.displayMessage m.X m.Y ConsoleColor.Yellow "<=")
    state

let displayEnemy state =
    match state.EnemyState with  
    | Vivo -> Utils.displayMessage state.EnemyX state.EnemyY ConsoleColor.Yellow "👾"
    | Colisionado -> Utils.displayMessage state.EnemyX state.EnemyY ConsoleColor.Yellow "💥" 
    | _ ->()
    state

let displayScore state =
    Utils.displayMessage 0 0 ConsoleColor.Yellow $"Score: {state.Puntaje}"
    state
...
let displayLives state =
    ...
    let textoVidas = String.Concat vidas
    Utils.displayMessage (state.Width/2-10) 0 ConsoleColor.Yellow $"Lives: {textoVidas}"
...
let clearAlien state =
    match state.AlienState with 
    | Vivo | Colisionado -> Utils.displayMessage state.AlienX state.AlienY ConsoleColor.Yellow "  "
    | _ -> ()
    state

let clearEnemy state =
    match state.EnemyState with 
    | Vivo | Colisionado -> Utils.displayMessage state.EnemyX state.EnemyY ConsoleColor.Yellow "  "
    | _ -> ()
    state
...
let clearMisil state =
    state.Misiles
    |> List.iter (fun m -> 
        Utils.displayMessage m.X m.Y ConsoleColor.Cyan "  "
    )
    state
